...
class NewsAnalyzer:
    ...
    def _fetch_shareholder_rewards_data(self) -> Optional[RSSData]:
        ...
        try:
            ...
            rewards_config = ShareholderRewardsConfig(
                ...
                max_items=self._config_int(cfg.get("MAX_ITEMS"), 30),
                max_age_days=self._config_int(cfg.get("MAX_AGE_DAYS"), 90),
                request_interval=self._config_int(cfg.get("REQUEST_INTERVAL"), 800),
                timeout=self._config_int(cfg.get("TIMEOUT"), 15),
                ...
                page_size=self._config_int(cfg.get("PAGE_SIZE"), 30),
            )

            if not rewards_config.keywords:
                print("[股东回馈] 未配置关键词，跳过抓取")
                return None

            return ShareholderRewardsFetcher(rewards_config).fetch_all()
    ...
    def _merge_rss_data(self, rss_data_sources: List[RSSData]) -> Optional[RSSData]:
        """合并标准 RSS 与特殊 RSS 数据源"""
        if not rss_data_sources:
            return None

        base = rss_data_sources[0]
        merged = RSSData(
            date=base.date,
            crawl_time=base.crawl_time,
            items={},
            id_to_name={},
            failed_ids=[],
        )
        ...
        for source in rss_data_sources:
            merged.id_to_name.update(source.id_to_name)

            for feed_id, items in source.items.items():
                merged.items.setdefault(feed_id, []).extend(items)

            for failed_id in source.failed_ids:
                if failed_id not in seen_failed:
                    merged.failed_ids.append(failed_id)
                    seen_failed.add(failed_id)

        if not merged.items and not merged.id_to_name and not merged.failed_ids:
            return None
    ...
    def _crawl_rss_data(self) -> Tuple[Optional[List[Dict]], Optional[List[Dict]], Optional[List[Dict]], set]:
        ...
        rss_data = self._merge_rss_data(rss_data_sources)
        if not rss_data:
            return None, None, None, set()

        self._rss_source_total = len(rss_data.id_to_name)
    ...
    def _convert_rss_items_to_list(self, items_dict: Dict, id_to_name: Dict) -> List[Dict]:
        ...
        rewards_config = self.ctx.shareholder_rewards_config
        rewards_feed_id = rewards_config.get("ID", "")
        rewards_max_age = rewards_config.get("MAX_AGE_DAYS")
